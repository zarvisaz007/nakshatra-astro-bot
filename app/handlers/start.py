...
_geocoder = Nominatim(user_agent="nakshatra-astro-bot")
...
def _gender_keyboard(lang: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=t(lang, "male"), callback_data="gender:male"),
        InlineKeyboardButton(text=t(lang, "female"), callback_data="gender:female"),
        InlineKeyboardButton(text=t(lang, "skip_gender"), callback_data="gender:skip"),
    ]])
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer(t("en", "choose_lang"), reply_markup=_lang_keyboard(), parse_mode="Markdown")
...
async def cb_language(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if u.is_onboarded:
        await callback.message.answer(
            t(lang, "welcome_back", name=u.name or ""),
...
async def cb_gender(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.answer()
    await callback.message.answer(t(lang, "ask_dob"), parse_mode="Markdown")
...
async def onboarding_dob(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(t(lang, "ask_tob"), reply_markup=_skip_kb(lang), parse_mode="Markdown")
    await state.set_state(Onboarding.birth_time)


@router.message(Onboarding.birth_time)
async def onboarding_tob(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    lang = data.get("lang", "en")
    ...
    await message.answer(t(lang, "ask_city"), reply_markup=ReplyKeyboardRemove(), parse_mode="Markdown")
...
async def onboarding_city(message: Message, state: FSMContext) -> None:
    ...
    async with AsyncSessionFactory() as session:
        await user_service.update_birth_data(
            ...
            city_name=city_name, timezone=tz_str,
            birth_time=birth_time, gender=gender,
        )

    await message.answer(t(lang, "generating_profile"))

    try:
        kundli = get_kundli(birth_date, lat, lon, birth_time, tz_str)
        intro = await horo_service.get_intro(
            name=name, lagna=kundli["lagna"], rashi=kundli["rashi"],
            nakshatra=kundli["nakshatra"], nak_lord=kundli["nakshatra_lord"],
            dasha=kundli["current_dasha"], lang=lang,
        )
    except Exception:
        logger.exception("Failed to generate intro for %s", name)
        intro = ""
